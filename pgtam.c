...
#define DEBUG_FUNC(format, ...) { \
		ereport(NOTICE, (errmsg(format, __VA_ARGS__))); \
}
...
static bool memam_getnextslot(TableScanDesc sscan, ScanDirection direction, TupleTableSlot *slot)
{
...
static bool memam_fetch_row_version(Relation relation, ItemPointer tid, Snapshot snapshot, TupleTableSlot *slot)
{
    DEBUG_FUNC("in %s", __func__);
    return true;
}
...
static bool memam_tuple_satisfies_snapshot(Relation rel, TupleTableSlot *slot, Snapshot snapshot)
{
    DEBUG_FUNC("in %s", __func__);
    return false;
}

static TransactionId memam_index_delete_tuples(Relation rel, TM_IndexDeleteOp *delstate)
{
    TransactionId id = InvalidTransactionId;
    DEBUG_FUNC("in %s", __func__);
    return id;
}
        ...
        MultiXactId *minmulti)
{
    ...
    DEBUG_FUNC("Created table: [%s]", table.name);
...
static void memam_relation_copy_data(Relation rel, const RelFileLocator *newrlocator)
{
    DEBUG_FUNC("in %s", __func__);
}
...
static void memam_vacuum_rel(Relation rel, VacuumParams *params, BufferAccessStrategy bstrategy)
{
    DEBUG_FUNC("in %s", __func__);
}

static bool memam_scan_analyze_next_block(TableScanDesc scan, BlockNumber blockno, BufferAccessStrategy bstrategy)
{
    DEBUG_FUNC("in %s", __func__);
    return false;
}
...
static bool memam_scan_sample_next_block(TableScanDesc scan, SampleScanState *scanstate)
{
    DEBUG_FUNC("in %s", __func__);
    return false;
}

static bool memam_scan_sample_next_tuple(TableScanDesc scan, SampleScanState *scanstate, TupleTableSlot *slot)
{
    DEBUG_FUNC("in %s", __func__);
    return false;
}
...
Datum mem_tableam_handler(PG_FUNCTION_ARGS)
{
    ...
    DEBUG_FUNC("in %s: mem_tableam handler loaded", __func__);
